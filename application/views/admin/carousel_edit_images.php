...
$action = 'admin_carousel/edit_carousel/0';
if(isset($carousel) && $carousel != false) {
	$disabled['checked'] = ($carousel->disabled == 1);
	$carousel_disabled = $carousel->disabled;
	$action = 'admin_carousel/edit_carousel/'.$id;
}

$action_image = 'admin_carousel/add_image/0';
if(isset($carousel) && $carousel != false) {
	$action_image = 'admin_carousel/add_image/'.$id;
}
			...
			if (count($images_array) > 0) {
				foreach($images_array as $img) {
					if($img->image_original_filename != "") {
						...
						echo '<img src="'.$image.'"/>';
						echo anchor('admin_carousel/remove_image/'.$id.'/'.$img->images_id, $lang['admin_removeimage']);
					}
					do_dump($img);
				}
			}
...
if(isset($carousel) && $carousel != false) {
	...
	<h2>Delete</h2>',
	form_open('admin_carousel/delete/'.$id),
