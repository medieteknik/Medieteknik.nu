...
foreach ($member_list as $member)
{
	...
	$member = array(anchor("user/profile/".$member->user_id,$member->first_name." ".$member->last_name), $member->position, $member->email,
					anchor('admin_groups/edit_member/'.$groups_year_id.'/'.$group_id.'/'.$member->user_id,
...
<div class="main-box clearfix profile">
	<h2>'.$lang['admin_groups_editmembers'].anchor('admin_groups/edit/'.$group_id, '&larr; '.$lang['misc_back']).'</h2>
	<p>',anchor('admin_groups/add_member/'.$groups_year_id.'/'.$group_id, $lang['admin_addmemberbyclicking']),'</p>',
...
if(isset($groups_year_id))
	echo form_open('admin_groups/remove_year/'.$groups_year_id);
else
	echo form_open('admin_groups/remove_year');
