...
class Page_model extends CI_Model
{
	...
	/**
	 * removes a page, must be draft (so no mistake is done)
	 * @param  int $id the page id
	 * @return [type]     [description]
	 */
	function admin_remove_page($id)
	{
		if($this->is_draft($id))
		{
			$translations = $this->db->delete('page_content', array('page_id' => $id));
			$page = $this->db->delete('page', array('id' => $id, 'published' => 0));
	...
	{
		$this->db->where(array('id' => $id, 'published' => 0));
		$q = $this->db->get('page');
